...
export default class extends Component {
  ...
  initEditor(props) {
    const policy_url = config.apiAppName(apis.getPolicyUrl);
    const auth = Cookie.get(config.cookie.auth_name);
    const group = 'group1';
  ...
  render() {
    ...
    return <div>
      <script id={this.props.id} name='myContent' type='text/plain' style={customStyle}></script>
    </div>
  }
}
