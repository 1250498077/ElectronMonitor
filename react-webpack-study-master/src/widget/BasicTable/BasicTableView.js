...
// 当前组件样式
import styles  from './BasicTableLess.less';
// 引入配置信息
...
class BasicTable extends Component {
  ...
  async getTableData(){
    ...
    try{
      ...
      // 取出父组件传来参数
      let table_data = get(self.props, 'tableProps.table_data', null)
  ...
  render(ReactElement, DOMElement, callback) {
    let fields = get(this.props, 'tableProps.fields', [])
    if(0 === fields.length){
      return ''
    }

    let columns = get(this.props, 'tableProps.columns', 3)

    // 字段列表转为二维数组
    let rowFields = this.one2TwoArray(fields, columns)
    return (
      <div className={`publicListBox ${styles.whiteBg}`}>
        <div className={cx(styles.tableDetail)}>
          {
            rowFields.map((row, i) => {
              return (
                <Row key={'row_' + i}>
